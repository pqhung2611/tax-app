...
# ====== GIAO DIỆN ======
st.set_page_config(page_title="Quyết toán thuế TNCN", page_icon="💰")

st.title("💰 App Quyết toán Thuế TNCN (Năm)")
st.write("Nhập thông tin để tính số thuế phải nộp / hoàn lại")

# Input
tong_thu_nhap = st.number_input("Tổng thu nhập chịu thuế (VND/năm)", min_value=0.0, step=1_000_000.0)
...
if auto_giam_tru:
    ...
    giam_tru = st.number_input("Tổng giảm trừ gia cảnh (VND/năm)", min_value=0.0, step=1_000_000.0)

bao_hiem = st.number_input("Bảo hiểm được trừ (VND/năm)", min_value=0.0, step=1_000_000.0)
thue_da_khau_tru = st.number_input("Thuế đã khấu trừ (VND)", min_value=0.0, step=1_000_000.0)
